...
pub fn sublist(first_list: &[i32], second_list: &[i32]) -> Comparison {
    ...
    } else if first_list.len() > second_list.len() && (second_list.is_empty() || boyer_moore(first_list, second_list)) {
        return Comparison::Superlist;
    } else if first_list.len() < second_list.len() && (first_list.is_empty() || boyer_moore(second_list, first_list)) {
