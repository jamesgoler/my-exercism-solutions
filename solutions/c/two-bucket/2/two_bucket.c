...
static inline struct queue create_queue(size_t size) {
    struct queue queue;
    queue.entries = malloc(sizeof(struct queue) + size * sizeof(struct queue_entry));
    queue.seens = calloc(size, sizeof(struct queue_entry));
    queue.cur = 0;
    queue.len = 0;
    queue.cap = size;
    return queue;
...
static inline bool is_empty(struct queue *queue) {
    return queue->cur == queue->cap;
}

static bucket_result_t bfs(struct queue *queue, bucket_liters_t goal_volume) {
    while (!is_empty(queue)) {
        struct queue_entry qe = pop_front_queue(queue);
        if (qe.bucket1 == 0 && qe.bucket2 == queue->bucket_size2) {
            continue;
        }
