...
static inline void init_product(product_t *p);
...
static inline int find_palindrome(int from, int to, factor_t **factor, int *palindrome)
{
    *factor = NULL;
        ...
        for (int j = i; j != end; j += steps) {
            ...
            if (*factor != NULL && product == *palindrome) {
                ...
                *factor = tmp;
                       ...
                       (find_min && *palindrome > product) || (!find_min && *palindrome < product)) {
